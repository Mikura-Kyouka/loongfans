...
您可能注意到 Steam 存在偶发的崩溃现象，这是因为默认版本的 Steam 运行时需要借助仍处于实验性阶段的 Box32 来运行。可以参考如下步骤切换到 SteamRT3 这一纯 64 位的运行时来规避由于上述原因导致的崩溃：

1. Steam > 界面 > 参与客户端测试：Steam Beta Update > 立即重启
2. （Steam 重启后）Steam > 界面 > 高级 > 使用实验性 SteamRT3 客户端 > 立即重启
